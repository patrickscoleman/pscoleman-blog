...
We used [TailwindCSS](https://tailwindcss.com/) for the CSS styles (colors, sizes, layout, etc.). It's a great way to get a good-looking and consistent app without needing to be a designer. [Utility classes](https://en.wikipedia.org/wiki/Tailwind_CSS) are just one way to do things, but I found it pretty easy and intuitive. It's worth spending time [customizing themes and colors](https://tailwindcss.com/docs/configuration) to make your life easier. Occasionally Tailwind didn't play nice with MUI (and vice versa)

### NextJS

[NextJS](https://nextjs.org/) is a great framework for building React web apps that comes with a lot of built-in features that make development much easier. It's also great for SEO and performance. It was created by Vercel, so works best when you deploy there (more below).

### Vercel

We used [Vercel](https://vercel.com/) to host the app. It's a great platform for hosting NextJS apps (and other serverless apps). It's easy to use, has great performance (see [SSR/SSG](https://vercel.com/blog/nextjs-server-side-rendering-vs-static-generation) and [Edge](https://vercel.com/docs/edge-network/overview)), and is free for small projects. Deploying is as easy as pushing to Github (more below) and the [basic analytics](https://vercel.com/docs/analytics) are solid too.
